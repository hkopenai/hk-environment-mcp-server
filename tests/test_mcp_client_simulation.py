...
This module contains unit tests to verify the functionality of MCP client tools.
"""
...
class TestMCPClientSimulation(unittest.TestCase):
    ...
    This class contains tests to verify the functionality of MCP client tools
    by running a live server and simulating client requests.
    """
    server_process = None
    ...
    def setUp(self):
        ...
        self.server_process = subprocess.Popen(
            [sys.executable, "-m", "hkopenai.hk_environment_mcp_server", "--sse"],
    ...
    def test_get_current_aqhi_tool(self):
        """
        Test the 'get_current_aqhi' tool by simulating a client request to the live server.
        Verifies that the tool returns a valid response without errors.
        """
        logger.debug("Testing 'get_current_aqhi' tool...")
        asyncio.run(self._call_tool_and_assert("get_current_aqhi", {}))
